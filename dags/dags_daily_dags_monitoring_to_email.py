...
with DAG(
    dag_id="dags_daily_dags_monitoring_to_email",
    schedule="0 8 * * *",
          ...
          with closing (postgres_hook.get_conn()) as conn :
               with closing(conn.cursor()) as cursor :
                    with open('/opt/airflow/files/sqls/daily_dag_monitoring.sql', 'r') as sql_file :
                         ...
                         html_content += "<h2>2 실패대상</h2><br/>"
                         if not failed_df.empty :
                           for idx, row in failed_df.iterrows() :
                             html_content += f"DAG: {row['dag_id']}<br/>최근 실패일자: {row['last_failed_date']}<br/>마지막 성공일자: {'없음' if str(row['last_success_date']) == 'NaT' else row['last_success_date']}"
                         else :
                           html_content += "없음<br/><br/>"
                         
                         # 2) 미수행 대상
                         skipped_df = rslt.query("(running_cnt == 0) ")
                         html_content += "<h2>3 미수행 대상</h2><br/>"                         
                         if not skipped_df.empty :
                           for idx, row in skipped_df.iterrows() :
                             html_content += f"DAG: {row['dag_id']}<br/>예정일자: {row['next_dagrun_data_interval_end']}"
                         else :
                           html_content += "없음<br/><br/>"
  
                         # 3) 수행 중 대상
                         running_df = rslt.query("(running_cnt > 0) ")
                         html_content += "<h2>4 수행중 </h2><br/>"    
                         if not running_df.empty :
                           for idx, row in running_df.iterrows() :
                             html_content += f"DAG: {row['dag_id']}<br/>배치일자: {row['next_dagrun_data_interval_start']}"
                         else :
                           html_content += "없음<br/><br/>"
